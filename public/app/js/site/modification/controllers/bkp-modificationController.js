...
ModificationModule.controller('modificationController', ['$scope', '$http', 'config', 'Flash', '$cookieStore', '$window', 'localStorageService', '$location', '$uibModal', '$sce', 'Auth',
    function ($scope, $http, config, Flash, $cookieStore, $window, localStorageService, $location, $uibModal, $sce, Auth) {
       ...
       localStorageService.set("disblelink", "disable-link");
        ...
        /* Set watch */
        $scope.$watch('editor_summary', function(newValue, oldValue) {
            ...
            if (newValue !== oldValue) {
                if(oldValue!=""){
                   ...
                   console.log('editor_summary');
                }
                
            }
        });
        $scope.$watch('editor_standards', function(newValue, oldValue) {
            var oldValue = oldValue || '';
             if(newValue == undefined){
                newValue = '';
            }
            ...
            if (newValue !== oldValue) {
                if(oldValue!=""){
                   $scope.disblelink = "enable";
                   localStorageService.set("disblelink","enable");
                   console.log('editor_standards');
                }
            }
        });

         $scope.$watch('editor_essential_questions', function(newValue, oldValue) {
            var oldValue = oldValue || '';
             if(newValue == undefined){
                newValue = '';
            }
            ...
            if (newValue !== oldValue) {
                if(oldValue!=""){
                   $scope.disblelink = "enable";
                   localStorageService.set("disblelink","enable");
                   console.log('editor_essential_questions');
                }
            }
        });

         $scope.$watch('editor_core_ideas', function(newValue, oldValue) {
            var oldValue = oldValue || '';
             if(newValue == undefined){
                newValue = '';
            }
            ...
            if (newValue !== oldValue) {
                if(oldValue!=""){
                   $scope.disblelink = "enable";
                   localStorageService.set("disblelink","enable");
                   console.log('editor_core_ideas');
                }
            }
        }); 
        
        $scope.$watch('editor_vocabulary', function(newValue, oldValue) {
            var oldValue = oldValue || '';
             if(newValue == undefined){
                newValue = '';
            }
            ...
            if (newValue !== oldValue) {
                if(oldValue!=""){
                   $scope.disblelink = "enable";
                   localStorageService.set("disblelink","enable");
                   console.log('editor_vocabulary');
                }
            }
        });

        $scope.$watch('lesson_modification', function(newValue, oldValue) {
            var oldValue = oldValue || '';
             if(newValue == undefined){
                newValue = '';
            }
        ...
        $scope.setPreview = function () {
            ...
            //console.log($scope.editor_summary); return false;
        ...
        $scope.getPreview = function () {
            ...
            console.log($scope.lesson_modification);
            ...
            if (!$scope.lesson_modification) {
                ...
                var Obj = {"last_lession_id": last_lession_id.last_id};
